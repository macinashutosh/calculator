...
function helloWorld(){
		...
		class CheckNeightbours{
			...
			addNeightbour(a,b,diff){
				if(diff > 0){
					...
					if(this.neighbour && this.neighbour.hasOwnProperty(nigga)){
						return false;
					}else{
						this.neighbour[nigga] = diff;
						return true;
					}
				}else{
					return false
	    		...
	    		lengthOfTheArray = lengthOfTheArray + getCombination(arr.length)
		    		...
		    		for (var i4 = 0;i4<arr.length;i4++){
		    			...
		    			if (sum >0 && temp.hasOwnProperty(sum)){
		    				...
		    				for(var j4=0;j4<toCheckTrue.length;j4++){
		    					isItCheckedAlready[toCheckTrue[j4]] = true
		    					var result1 = neighbour.addNeightbour(i,toCheckTrue[j4],sum)
		   						if(result1 === true){
		    						var toShow1 = arr[toCheckTrue[j4]].getInfo();
		   						}
		    				}
		    			}
		    		}
	...
	// PrintAnswer Ends here
  	...
  	var inputArray = [4,6,8,10,12, 7 ,5,7,9,11, 6 , 4, 6];
    ...
    console.log("First Base Unit Size Calculated ==>  " + BaseUnit);
        ...
        console.log("\n");
			...
			// console.log(permutedResultsTemp)
			// console.log(permutedResultsTemp)

			// console.log(lengthOfTheArray)

			// console.log("Minimum FWM:"+currentSum);
			// console.log("Final Array"+finalArray);

      // res.status(200).send('Success: ' + permutedResults.length);
      // return 'Success: ' + permutedResultsTemp.length;
      return 'success';
